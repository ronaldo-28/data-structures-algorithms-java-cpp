  ...
  int minJumps(std::vector<int> &arr) {
    ...
    while (true) {
      ...
      if (/*left >= 0 && */ depth[left] == 0) {
        ...
        // std::cout << " DEQ " << "Adding left index: " << left << "\n";
        deq.push_back(left);
      }
      ...
      for (auto &c : connections) {
        // if (depth_mirror[c] != 0) {
        //   return depth[index] + depth_mirror[c] - 1;
        // }

        if (depth[c] == 0) {
          depth[c] = depth[index] + 1;
          // std::cout << " DEQ " << "Adding index: " << c
          //           << " with the same value: " << arr[index] << "\n";
          deq.push_back(c);
        }
      }
      connections.clear();

      {
        ...
        // check if the right index trace is found
        // if (/*right < arr.size() && */ depth[right] != 0) {
        //   return depth_mirror[index_mirror] + depth[right] - 1;
        // }

        if (/*right < arr.size() && */ depth_mirror[right] == 0) {
          depth_mirror[right] = depth_mirror[index_mirror] + 1;
          // std::cout << " DEQ MIRROR " << "Adding right index: " << right <<
          // "\n";
        ...
        // if (/*left >= 0 && */ depth[left] != 0) {
        //   return depth_mirror[index_mirror] + depth[left] - 1;
        // }

        if (/*left >= 0 && */ depth_mirror[left] == 0) {
          depth_mirror[left] = depth_mirror[index_mirror] + 1;
          // std::cout << " DEQ MIRROR " << "Adding left index: " << left <<
          // "\n";
        ...
        for (auto &c : connections) {
          // if (depth[c] != 0) {
          //   return depth_mirror[index_mirror] + depth[c] - 1;
          // }

          if (depth_mirror[c] == 0) {
            depth_mirror[c] = depth_mirror[index_mirror] + 1;
            // std::cout << " DEQ MIRROR " << "Adding index: " << c
            //           << " with the same value: " << arr[index_mirror] <<
            //           "\n";
            deq_mirror.push_back(c);
          }
        }
