class Solution {
    ...
    public boolean[] canMakePalindromeQueries(String s, int[][] queries) {
        ...
        for (int i = problemPoint; i < n / 2; i++) {
            ...
            while (mp.containsKey(s.charAt(rptr)) || (rptr >= n / 2 && s.charAt(rptr) == s.charAt(opp(rptr)) && mp.size() == 0)) {
        ...
        for (int i = opp(problemPoint); i >= n / 2; i--) {
            ...
            while (mp.containsKey(s.charAt(lptr)) || (lptr < n / 2 && s.charAt(lptr) == s.charAt(opp(lptr)) && mp.size() == 0)) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            // if either interval covers the problematic interval on its side, it does not matter
